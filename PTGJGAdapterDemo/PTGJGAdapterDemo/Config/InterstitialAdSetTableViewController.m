...
@interface InterstitialAdSetTableViewController ()

@property (nonatomic, strong) UITextField *textField;

@end

@implementation InterstitialAdSetTableViewController
...
- (void)clickSaveButton {
    if (self.textField.text != nil)
        [SetConfigManager sharedManager].insterstitialAdScenceId = self.textField.text;
}
...
//        _textField.placeholder = @"请输入";
        ...
        if (![[SetConfigManager sharedManager].insterstitialAdScenceId isEqualToString:@""]) {
            _textField.text = [SetConfigManager sharedManager].insterstitialAdScenceId;
        }
    }
    return _textField;
}
